...
getRefParentOrDiag(const clang::RecordDecl *decl, ASTContext &ctx,
  ...
  if (uniqueRetainDecls.size() != 1 || uniqueReleaseDecls.size() != 1) {
    if (importerImplPtr) {
      if (importerImplPtr->DiagnosedCxxRefDecls.find(decl) ==
          importerImplPtr->DiagnosedCxxRefDecls.end()) {
        HeaderLoc loc(decl->getLocation());
        importerImplPtr->diagnose(loc, diag::cant_infer_frt_in_cxx_inheritance,
                                  decl);
        importerImplPtr->DiagnosedCxxRefDecls.insert(decl);
      }
    } else {
      ctx.Diags.diagnose({}, diag::cant_infer_frt_in_cxx_inheritance, decl);
      assert(false && "nullpointer passeed for importerImplPtr when calling "
                      "getRefParentOrDiag");
    }
    return nullptr;
  }
...
CxxRecordSemantics::evaluate(Evaluator &evaluator,
  ...
  ClangImporter::Implementation *importerImplPtr = desc.importerImplPtr;
  if (hasImportAsRefAttr(decl) ||
      getRefParentOrDiag(decl, desc.ctx, importerImplPtr)) {
  ...
  if (!hasDestroyTypeOperations(cxxDecl) ||
    ...
    if (desc.shouldDiagnoseLifetimeOperations) {
      ...
      if (hasUnsafeAPIAttr(cxxDecl))
        importerImplPtr->diagnose(loc, diag::api_pattern_attr_ignored,
...
CustomRefCountingOperationResult CustomRefCountingOperation::evaluate(
  ...
  if (!hasImportAsRefAttr(decl)) {
    if (auto parentRefDecl = getRefParentOrDiag(decl, ctx)) {
      decl = parentRefDecl;
    }
  }
